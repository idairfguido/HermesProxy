...
| Quest area highlight on world map (POI blob mesh) | ❌ | ❓ | 2026-05-09 — quest objective dots show on the world map but the translucent blue area-mesh that highlights "where this quest can be completed" is missing. Translator + writer are in place — see Open issues |
...
- **World-map quest area highlight (blue POI blob mesh) does not render — TC.** When the user opens the **world map** with a quest tracked, the per-objective dots render correctly but the translucent blue area-mesh that the V3_4_3 client uses to indicate "where this quest can be completed in this zone" is absent. The data path is in place: `CMSG_QUEST_POI_QUERY` is forwarded to the legacy server (`HermesProxy/World/Server/PacketHandlers/QuestHandler.cs:100`), the legacy `SMSG_QUEST_POI_QUERY_RESPONSE` is parsed and translated to V3_4_3 layout (`HermesProxy/World/Client/PacketHandlers/QuestHandler.cs:510`) including `WorldMapAreaID → UiMapID` translation via `GameData.GetUiMapId`, and `QuestPOIQueryResponse.Write` (`HermesProxy/World/Server/Packets/QuestPackets.cs:434`) emits all 13 V3_4_3 blob fields + the `AlwaysAllowMergingBlobs` bit. Likely candidates for the gap: (a) point coordinates — translator does `(short)packet.ReadInt32()` which silently truncates if the legacy server emits values outside `int16` range (legacy uses raw map-coord int32; V3_4_3 uses int16 in a different coordinate space), so the polygon could be collapsing to a degenerate point; (b) `UiMapID` mapping has holes for low-level zones (Durotar, Elwynn etc.) and a `0` UiMapID makes the V3_4_3 client drop the blob; (c) zero-default fields the V3_4_3 client treats as render gates — `Priority`, `Flags`, `WorldEffectID`, `PlayerConditionID`, `NavigationPlayerConditionID`, `SpawnTrackingID`, `QuestObjectiveID`, `QuestObjectID` are all left at 0 by the translator (legacy `SMSG_QUEST_POI_QUERY_RESPONSE` doesn't carry them), and one of these may be required-non-zero for the mesh to render. Triage: WPP-parse a captured V3_4_3 native `SMSG_QUEST_POI_QUERY_RESPONSE` for a known-good zone (e.g. Borean Tundra) and diff against our translator's output to identify which zero-default field carries load-bearing data. cMangos untested.
